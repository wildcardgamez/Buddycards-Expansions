...
public class CardModelGen extends ItemModelProvider {
    ...
    protected void registerModels() {
        for (BuddycardItem card: BuddycardsAPI.getAllCards())
            if(!card.getSet().getName().equals("base") && !card.getSet().getName().equals("nether") && !card.getSet().getName().equals("end"))
                genCardModel(card.getSet().getName(), card.getCardNumber());
    }
    ...
    void genCardModel(String setName, int cardNum) {
        final ResourceLocation location = new ResourceLocation(BuddycardsExpansions.MOD_ID, ModelProvider.ITEM_FOLDER + "/buddycard_" + setName + cardNum);
        ItemModelBuilder card = factory.apply(location)
                ...
                .texture("layer0", new ResourceLocation(BuddycardsExpansions.MOD_ID, ModelProvider.ITEM_FOLDER + "/" + setName + "_set/" + cardNum));
    ...
    ModelFile genGradedCardModel(String setName, int cardNum, int grade) {
        ...
        location = new ResourceLocation(BuddycardsExpansions.MOD_ID, ModelProvider.ITEM_FOLDER + "/buddycard_" + setName + cardNum + "_g" + grade);
        ItemModelBuilder card = factory.apply(location)
                .parent(factory.apply(new ResourceLocation(Buddycards.MOD_ID, ModelProvider.ITEM_FOLDER + "/buddycard")))
                .texture("layer0", new ResourceLocation(BuddycardsExpansions.MOD_ID, ModelProvider.ITEM_FOLDER + "/" + setName + "_set/" + cardNum));
